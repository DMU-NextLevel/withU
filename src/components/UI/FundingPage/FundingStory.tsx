...
export const FundingNews = (): JSX.Element => {
	const exam = [
		...
		{id:2, title:"프로젝트 FAQ 궁금해 하실 부분들을 말씀드릴게요", content:`안녕하세요, oooo입니다. 여러부늘이 관심을 가져주시고 계시는 저희 프로젝트에 대해 설명드리겠습니다 \n\n\n이건이거구요 저건저거구요`}
	]
    ...
    return (
        <InfoWrapper>
			...
			<Bar/>
			{exam.map((news) => (<NewsContent title={news.title} content={news.content}/>))}
        </InfoWrapper>
    )
}

export const FundingCommu = (): JSX.Element => {
	const exam = [
		{
			isSender: false,
	...
	return (
		<CommuWrapper>
			...
			<Bar/>
			{exam.map((commu) => (<FundingMessage isSender={commu.isSender} message={commu.message} date={commu.date} userName={commu.userName} />))}
		</CommuWrapper>
	)
}
